...
class ChatConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        await self.channel_layer.group_add("chats", self.channel_name) #type:ignore
        await self.accept()



    async def disconnect(self, close_code):
        await self.channel_layer.group_discard(self.room_name, self.channel_name) #type:ignore
    ...
    async def receive(self, text_data):
        ...
        if type_of_msg == "connect":
            ...
            text = data["text"]
            msg = await create_msg(self.curr_room, self.user, self.other_user, text) #type:ignore
            ...
            msg = await create_document(self.curr_room, self.user, self.other_user, file_data, file_name) #type:ignore
            await self.channel_layer.group_send( #type:ignore
                self.room_name,
                {
                    "type": "chat_message",
